...
export function CandidateDetailsDialog({ candidate, isLoading, onClose }: CandidateDetailsDialogProps) {
  ...
  return (
    <dialog ref={dialogRef} className="candidate-dialog" aria-labelledby="candidate-details-title" onCancel={onClose}>
      <header className="candidate-dialog__header">
        <h2 id="candidate-details-title">Candidate details</h2>
        <button type="button" className="candidate-dialog__close" aria-label="Close dialog" onClick={onClose}>
          ✕
        </button>
      </header>
        ...
        {isLoading && !candidate ? (
          ...
          candidate && (
            <dl className="candidate-details">
              <div><dt>Student Name</dt><dd>{candidate.studentFullName}</dd></div>
              <div><dt>Registration Number</dt><dd>{candidate.studentRegistrationNumber}</dd></div>
              <div className="candidate-details__wide"><dt>Email</dt><dd>{candidate.studentEmail}</dd></div>
              <div><dt>Election</dt><dd>{candidate.electionTitle}</dd></div>
              <div><dt>Election Status</dt><dd>{candidate.electionStatus}</dd></div>
              <div><dt>Approval Status</dt><dd><CandidateStatusBadge status={candidate.status} /></dd></div>
              <div><dt>Applied At</dt><dd>{formatUtcDateTime(candidate.nominatedAt)}</dd></div>
              <div className="candidate-details__wide">
                <dt>Manifesto</dt>
                <dd className="candidate-details__manifesto">{displayOptional(candidate.manifesto)}</dd>
              </div>
      ...
      <footer className="candidate-dialog__actions">

...
export function StudentDetailsDialog({ student, isLoading, onClose }: StudentDetailsDialogProps) {
  ...
  return (
    <dialog ref={dialogRef} className="student-dialog" aria-labelledby="student-details-title" onCancel={onClose} onClose={onClose}>
      <header className="student-dialog__header">
        <h2 id="student-details-title">Student details</h2>
        <button type="button" className="student-dialog__close" aria-label="Close dialog" onClick={onClose}>
          ✕
        </button>
      </header>
        ...
        {isLoading && !student ? (
          ...
          student && (
            <dl className="student-details">
              ...
              <div><dt>Registration Number</dt><dd>{student.registrationNumber}</dd></div>
              <div><dt>Email</dt><dd>{student.email}</dd></div>
              <div><dt>Department</dt><dd>{student.department}</dd></div>
              <div><dt>Year of Study</dt><dd>{student.yearOfStudy}</dd></div>
              <div><dt>Gender</dt><dd>{formatGender(student.gender)}</dd></div>
              <div><dt>Phone Number</dt><dd>{displayOptional(student.phoneNumber)}</dd></div>
              <div><dt>Approval Status</dt><dd>{student.approvalStatus}</dd></div>
      ...
      <footer className="student-dialog__actions">

...
export function PendingStudentsTable({
    ...
    () =>
      columnHelper.columns([
        ...
        columnHelper.accessor('approvalStatus', {
          header: 'Status',
          cell: (context) => (
            <span className="pending-students-table__status">{context.getValue()}</span>
          ),
        }),
  ...
  if (isLoading)
    return (
      <div className="pending-students-table__state">
        <LoadingSpinner label="Loading pending students" />
      </div>
    )
  if (students.length === 0)
    return (
      <p className="pending-students-table__state">
        {hasSearch ? 'No students match your search.' : 'No pending student registrations found.'}
      </p>
    )

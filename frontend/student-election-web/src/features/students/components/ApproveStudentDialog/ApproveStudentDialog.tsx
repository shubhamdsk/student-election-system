  ...
  return (
    ...
    >
      <header className="student-dialog__header">
        <h2 id="approve-student-title">Approve student?</h2>
        <button
          type="button"
      ...
      <div className="student-dialog__content">
        <p className="student-dialog__message">
          Are you sure you want to approve {student?.fullName ?? 'this student'}'s registration?
        </p>
      </div>
      <div className="student-dialog__actions">
        <Button variant="secondary" disabled={isSubmitting} onClick={onCancel}>Cancel</Button>
        <Button variant="success" disabled={isSubmitting} isLoading={isSubmitting} loadingLabel="Approving" onClick={onConfirm}>
          Approve
        </Button>
      </div>
    </dialog>

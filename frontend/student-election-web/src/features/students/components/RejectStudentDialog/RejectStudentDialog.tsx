    ...
    >
      <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
        <header className="student-dialog__header">
          <h2 id="reject-student-title">Reject student?</h2>
          <button
            type="button"
            className="student-dialog__close"
        ...
        <div className="student-dialog__content">
          <div className="student-dialog__field">
            <label htmlFor="rejection-reason">
              Reason for rejecting {student?.fullName ?? 'student'} <span aria-hidden="true">*</span>
            </label>
            <textarea
              id="rejection-reason"
              value={values.reason}
              ...
              aria-describedby="rejection-reason-error rejection-reason-count"
              onChange={(event) => { setValues({ reason: event.target.value }); setError('') }}
            />
            <p id="rejection-reason-error" className="student-dialog__error" aria-live="polite">
              {error && `Error: ${error}`}
            </p>
            <p id="rejection-reason-count" className="student-dialog__count">
              {values.reason.length}/500
            </p>
          </div>
        </div>
        <div className="student-dialog__actions">
